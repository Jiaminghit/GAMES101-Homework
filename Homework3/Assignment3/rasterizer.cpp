                ...
                for (float i = 0.25; i < 1.0; i += 0.5) {
                    for (float j = 0.25; j <=1.0; j += 0.5) {
                        if (insideTriangle(x + i, y + j, t.v)){
                            ...
                            if (abs(z_interpolated) < depth_buf_2xMSAA[get_index(x, y)][index]) {
                                ...
                                frame_buf_2xMSAA[get_index(x, y)][index] = pixel_color;
                                depth_buf_2xMSAA[get_index(x, y)][index] = abs(z_interpolated);
                            }
                        }
                        index++;
                    }
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    if ((buff & rst::Buffers::Color) == rst::Buffers::Color)
    {
        ...
        for (int i = 0; i < frame_buf_2xMSAA.size(); i++) {
            frame_buf_2xMSAA[i].resize(4);
            std::fill(frame_buf_2xMSAA[i].begin(), frame_buf_2xMSAA[i].end(), Eigen::Vector3f(0, 0, 0));
        }
    }
    if ((buff & rst::Buffers::Depth) == rst::Buffers::Depth)
    {
        std::fill(depth_buf.begin(), depth_buf.end(), std::numeric_limits<float>::infinity());
        for (int i = 0; i < depth_buf_2xMSAA.size(); i++) {
            depth_buf_2xMSAA[i].resize(4);
            std::fill(depth_buf_2xMSAA[i].begin(), depth_buf_2xMSAA[i].end(), std::numeric_limits<float>::infinity());
        }
    }
}
